...
document.addEventListener('alpine:init', () => {
    Alpine.data('storefront', () => ({
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (this.filters.sort === 'price_asc')
                        items = items.sort((a, b) => (a.base_price ?? 0) - (b.base_price ?? 0));
                    if (this.filters.sort === 'price_desc')
                        items = items.sort((a, b) => (b.base_price ?? 0) - (a.base_price ?? 0));
        ...
        async ensureVariants(productId) {
            ...
            try {
                const res = await this.api(`/products/${productId}/variants`, { method: 'GET' });
                if (res.ok) {
        ...
        cartTotal() {
            return this.cart.reduce((sum, item) => sum + (Number(item.qty) * Number(item.price)), 0);
        },
        ...
        async loadOrders() {
            ...
            if (res.ok) this.orders = res.json?.data || res.json || [];
        },
